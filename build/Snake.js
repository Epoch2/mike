  ...
  MC.Snake = (function() {
    function Snake(position, color, name) {
      ...
      for (i = _i = 0, _ref = num - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
        ...
        this.particles.push(new MC.Particle(pos, rad, color, new MC.Vec2(0, 0)));
      }
      this.particles[0].head = true;
      this.head = this.particles[0];
      for (i = _j = 0, _ref1 = num - 2; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
        this.springs.push(new MC.Spring(this.particles[i], this.particles[i + 1], springConst, springLen, springFriction));
      }
    }
    ...
    Snake.prototype.render = function(blending) {
      ...
      ctx.fillStyle = "#fff";
      ctx.opa;
      pos = this.particles[0].currPos.plus(new MC.Vec2(this.name.split("").length * -3, -20));
    ...
    Snake.prototype.update = function(dt) {
      ...
      if (this.right && this.move) {
        this.dir.rotate(MC.Maths.toRadians(-0.5));
      }
      if (this.left && this.move) {
        this.dir.rotate(MC.Maths.toRadians(0.5));
      }
      if (this.move) {
        this.dir.rotate(MC.Maths.toRadians(Math.sin(this.iterations += 0.04)));
      }
      if (this.move) {
